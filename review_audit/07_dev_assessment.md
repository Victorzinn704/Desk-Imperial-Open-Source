...
| Segurança aplicada | **4.0** | Cookie __Host- prefix, Argon2id, CSRF double-submit com timing-safe compare, CORS restritivo, HTTPS enforcement, helmet, rate limiting. Vulnerabilidades npm zeradas. Gaps: enumeração de usuário, sem rate limit no registro. |
...
2. **Disciplina de segurança real, não cosmética.** Cookie __Host-, Argon2id, timing-safe compare para CSRF. Isso demonstra leitura de specs, não copy-paste de tutorial.
