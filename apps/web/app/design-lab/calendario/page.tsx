  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <button
          ...
          style={{
            ...
            background: 'var(--lab-blue)', color: 'white', border: 'none', borderRadius: 8,
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {(Object.entries(EVENT_CONFIG) as [EventType, { label: string; color: string; bg: string }][]).map(([type, cfg]) => (
          <span key={type} style={{ fontSize: 11, fontWeight: 500, color: cfg.color, background: cfg.bg, borderRadius: 20, padding: '3px 10px' }}>
            {cfg.label}
          </span>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_300px]">
        <div className="lab-card">
          <div className="lab-card-header">
            ...
            <h2 className="lab-card-title" style={{ margin: '0 auto' }}>Abril 2026</h2>
            <button className="lab-icon-btn" type="button" style={{ border: '1px solid var(--lab-border)' }}>
          ...
          <div className="lab-card-p" style={{ paddingTop: 0 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 4 }}>
              {DAYS_OF_WEEK.map((d) => (
                <div key={d} style={{ textAlign: 'center', fontSize: 11, fontWeight: 600, color: 'var(--lab-fg-muted)', padding: '4px 0' }}>{d}</div>
              ))}
            </div>
              ...
              {cells.map((day, idx) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      minHeight: 64, borderRadius: 8, padding: 6,
                      ...
                      cursor: day ? 'pointer' : 'default', textAlign: 'left', transition: 'all 0.15s',
                    }}
                  >
                    {day && (
                      <>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          {dayEvents.slice(0, 2).map((ev) => {
                            ...
                            return (
                              <div key={ev.id} style={{ fontSize: 9, fontWeight: 600, color: cfg.color, background: cfg.bg, borderRadius: 3, padding: '1px 4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {ev.title}
                              </div>
                            )
                          })}
                          {dayEvents.length > 2 && <div style={{ fontSize: 9, color: 'var(--lab-fg-muted)', paddingLeft: 2 }}>+{dayEvents.length - 2}</div>}
                        </div>
                      </>
                    )}
                  </button>
        ...
        <div className="lab-card">
          <div className="lab-card-header">
            <div>
              ...
              <p className="lab-card-subtitle">{selected ? `${selectedEvents.length} evento${selectedEvents.length !== 1 ? 's' : ''}` : ''}</p>
            </div>
          </div>
          <div className="lab-card-p flex flex-col gap-3">
            {selectedEvents.length === 0 && (
              <p style={{ fontSize: 13, color: 'var(--lab-fg-muted)', textAlign: 'center', padding: '24px 0' }}>Nenhum evento neste dia</p>
            )}
            {selectedEvents.map((ev) => {
              const cfg = EVENT_CONFIG[ev.type]
              return (
                <div key={ev.id} style={{ borderRadius: 8, padding: 12, background: cfg.bg, border: `1px solid ${cfg.color}30` }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontSize: 10, fontWeight: 600, color: cfg.color, background: `${cfg.color}22`, borderRadius: 4, padding: '1px 6px' }}>{cfg.label}</span>
                    {ev.impact && <span style={{ fontSize: 11, fontWeight: 700, color: cfg.color }}>{ev.impact} vendas</span>}
                  </div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--lab-fg)', margin: '4px 0 2px' }}>{ev.title}</p>
          ...
          <div style={{ borderTop: '1px solid var(--lab-border)', padding: '12px 16px' }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--lab-fg-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Próximos</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {EVENTS.filter((e) => e.day > (selected ?? 0)).slice(0, 4).map((ev) => {
                const cfg = EVENT_CONFIG[ev.type]
                return (
                  <div key={ev.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: cfg.color, flexShrink: 0 }} />
                    <p style={{ flex: 1, fontSize: 12, fontWeight: 500, color: 'var(--lab-fg)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ev.title}</p>
