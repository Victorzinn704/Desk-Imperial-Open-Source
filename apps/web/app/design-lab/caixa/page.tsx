...
import { LabPageHeader, LabPanel, LabSignalRow, LabStatusPill } from '@/components/design-lab/lab-primitives'
...
export default function DesignLabCaixaPage() {
  ...
  const showSummaryPanels =
    !(operationsQuery.isLoading || errorMessage) &&
    Boolean(
  ...
  return (
    <section className="space-y-5">
      <LabPageHeader description="Abertura, fechamento e conferência." eyebrow="Operação de caixa" title="Caixa" />
      ...
      {user && isInitialLoading ? (
        <LabPanel action={<LabStatusPill tone="info">sincronizando</LabStatusPill>} padding="md" title="Caixa do dia">
          <div className="space-y-0">
            <LabSignalRow label="receita" note="lendo fechamentos já consolidados" tone="neutral" value="aguarde" />
            <LabSignalRow
              label="caixa esperado"
              note="buscando referência do fechamento atual"
              tone="info"
              value="carregando"
            />
            <LabSignalRow
              label="comandas abertas"
              note="checando pendências antes do encerramento"
              tone="neutral"
              value="aguarde"
            />
          </div>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-start">
      ...
      >
        ...
        <div className="mt-5 space-y-0">
          <LabSignalRow
            label="abrir caixa"
            note="o login libera a abertura real do turno"
            tone="info"
            value="ao entrar"
          />
          <LabSignalRow
            label="fechamento"
            note="conferência e diferença só aparecem com sessão ativa"
            tone="neutral"
            value="bloqueado"
          />
          <LabSignalRow
            label="próxima ação"
            note="o painel volta a mostrar caixa esperado, lucro e pendências"
            tone="warning"
            value="autenticar"
          />
        </div>
      ...
      <LabPanel action={<LabStatusPill tone="info">preview</LabStatusPill>} padding="md" title="O que abre no caixa">
        <div className="space-y-0">
          <LabSignalRow
            label="abertura"
            note="define valor inicial e observações do turno"
            tone="neutral"
            value="sim"
          />
          <LabSignalRow
            label="fechamento"
            note="confere contado, diferença e bloqueios por comanda"
            tone="success"
            value="sim"
          />
          ...
          <LabSignalRow
            label="controle real"
            note="o CTA operacional deixa de ser só visual"
            tone="neutral"
            value="sim"
          />
        </div>
