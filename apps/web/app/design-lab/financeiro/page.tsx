...
const BAR_OPTIONS: ApexOptions = {
  ...
  yaxis: {
    labels: { style: { colors: '#636363', fontSize: '11px' }, formatter: (v) => `${(v / 1000).toFixed(0)}k` },
  },
...
const TRANSACTIONS = [
  ...
  { id: '#T-0088', desc: 'Vendas — Dia 06/04', tipo: 'entrada', valor: 'R$ 4.820,00', data: '06/04', status: 'confirmado' },
  { id: '#T-0087', desc: 'Folha de Pagamento — Março', tipo: 'saída', valor: 'R$ 12.400,00', data: '05/04', status: 'pago' },
  { id: '#T-0086', desc: 'Vendas — Dia 05/04', tipo: 'entrada', valor: 'R$ 3.910,00', data: '05/04', status: 'confirmado' },
  { id: '#T-0085', desc: 'Manutenção Equipamentos', tipo: 'saída', valor: 'R$ 680,00', data: '04/04', status: 'pago' },
  { id: '#T-0084', desc: 'Vendas — Dia 04/04', tipo: 'entrada', valor: 'R$ 5.240,00', data: '04/04', status: 'confirmado' },
]
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_360px]">
        <div className="lab-card">
          <div className="lab-card-header">
            ...
            <button className="lab-icon-btn" type="button"><MoreHorizontal className="size-4" /></button>
          </div>
          <div className="p-4">
            ...
            {CATEGORIES.map((c) => (
              <div key={c.name} className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  ...
                  <span style={{ fontSize: 11, color: 'var(--lab-fg-muted)' }}>{c.revenue} · {c.pct}%</span>
                </div>
      ...
      <div className="lab-card">
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="lab-table">
            <thead>
              <tr><th>ID</th><th>Descrição</th><th>Tipo</th><th>Valor</th><th>Data</th><th>Status</th></tr>
            </thead>
            <tbody>
              {TRANSACTIONS.map((t) => (
                <tr key={t.id}>
                  ...
                  <td className="lab-td-main" style={{ color: t.tipo === 'entrada' ? '#22c55e' : '#f87171' }}>
                    {t.tipo === 'saída' ? '-' : '+'}{t.valor}
                  </td>
                  <td style={{ color: 'var(--lab-fg-muted)', fontSize: 12 }}>{t.data}</td>
                  <td><span className="lab-pill lab-pill--green">{t.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
