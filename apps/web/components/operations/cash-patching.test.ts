...
import {
  cashSession, liveSnapshot, kitchenSnapshot, summarySnapshot,
...
describe('cash.updated', () => {
  it('patches employee cash session by full record', () => {
    const live = liveSnapshot({ employees: [{ employeeId: 'emp-1', employeeCode: 'E01', displayName: 'Joao', active: true, cashSession: cashSession(), comandas: [] }] })
    const mock = createQueryClientMock(live)
    applyRealtimeEnvelope(qc(mock), { event: 'cash.updated', payload: { businessDate: '2026-03-30', cashSession: cashSession({ expectedCashAmount: 500 }) } })
  ...
  it('patches employee cash session by cashSessionId (partial fields)', () => {
    const live = liveSnapshot({ employees: [{ employeeId: 'emp-1', employeeCode: 'E01', displayName: 'Joao', active: true, cashSession: cashSession({ id: 'cs-1', expectedCashAmount: 200 }), comandas: [] }] })
    const mock = createQueryClientMock(live)
    applyRealtimeEnvelope(qc(mock), { event: 'cash.updated', payload: { businessDate: '2026-03-30', cashSessionId: 'cs-1', expectedAmount: 350, openingAmount: 120, status: 'OPEN' } })
  ...
  it('patches unassigned cash session by cashSessionId', () => {
    const live = liveSnapshot({ unassigned: { employeeId: null, employeeCode: null, displayName: 'Owner', active: true, cashSession: cashSession({ id: 'cs-unassigned', employeeId: null, expectedCashAmount: 100 }), comandas: [] } })
    const mock = createQueryClientMock(live)
    applyRealtimeEnvelope(qc(mock), { event: 'cash.updated', payload: { businessDate: '2026-03-30', cashSessionId: 'cs-unassigned', expectedAmount: 999 } })
  ...
  it('patches counted and difference amounts via cashSessionId', () => {
    const live = liveSnapshot({ employees: [{ employeeId: 'emp-1', employeeCode: 'E01', displayName: 'Joao', active: true, cashSession: cashSession({ id: 'cs-1' }), comandas: [] }] })
    const mock = createQueryClientMock(live)
    applyRealtimeEnvelope(qc(mock), { event: 'cash.updated', payload: { businessDate: '2026-03-30', cashSessionId: 'cs-1', countedAmount: 195, differenceAmount: -5, status: 'CLOSED' } })
    const cs = mock.getLiveSnapshot().employees[0].cashSession
  ...
  it('handles full record with employeeId not matching any group gracefully', () => {
    ...
    const mock = createQueryClientMock(live)
    applyRealtimeEnvelope(qc(mock), { event: 'cash.updated', payload: { businessDate: '2026-03-30', cashSession: cashSession({ employeeId: 'emp-unknown' }) } })
...
describe('cash.opened', () => {
  it('sets cash session on matching employee group', () => {
    const live = liveSnapshot({ employees: [{ employeeId: 'emp-1', employeeCode: 'E01', displayName: 'Joao', active: true, cashSession: null, comandas: [] }] })
    const mock = createQueryClientMock(live)
    applyRealtimeEnvelope(qc(mock), { event: 'cash.opened', payload: { businessDate: '2026-03-30', cashSession: cashSession({ id: 'cs-new' }) } })
  ...
  it('sets cash session on unassigned when employeeId is null', () => {
    const mock = createQueryClientMock(liveSnapshot())
    applyRealtimeEnvelope(qc(mock), { event: 'cash.opened', payload: { businessDate: '2026-03-30', cashSession: cashSession({ id: 'cs-owner', employeeId: null }) } })
...
describe('cash.closure.updated', () => {
  it('patches closure fields in live snapshot', () => {
    ...
    applyRealtimeEnvelope(qc(mock), {
      ...
      payload: { businessDate: '2026-03-30', status: 'PENDING', expectedAmount: 999, grossRevenueAmount: 500, realizedProfitAmount: 200, pendingCashSessions: 3, openComandasCount: 7, countedAmount: 990, differenceAmount: -9 },
    })
  ...
  it('returns liveNeedsRefresh when snapshot has no closure', () => {
    ...
    const result = applyRealtimeEnvelope(qc(mock), { event: 'cash.closure.updated', payload: { businessDate: '2026-03-30', status: 'CLOSED', expectedAmount: 1 } })
    expect(result.liveNeedsRefresh).toBe(true)
  })
  ...
  it.each([
    ...
    const mock = createQueryClientMock(liveSnapshot())
    applyRealtimeEnvelope(qc(mock), { event: 'cash.closure.updated', payload: { businessDate: '2026-03-30', status: input, expectedAmount: 1 } })
  ...
  it('patches summary kpis with projecaoTotal and lucroEsperado', () => {
    ...
    applyRealtimeEnvelope(qc(mock), {
      event: 'cash.closure.updated',
      payload: { closureId: 'cl-1', status: 'CLOSED', expectedAmount: 130, grossRevenueAmount: 180, realizedProfitAmount: 55, countedAmount: 131, differenceAmount: 1, openComandasCount: 0, pendingCashSessions: 0, businessDate: '2026-03-30' },
    })
  ...
  it('handles zero receitaRealizada in summary by setting estimatedOpenMargin to 0', () => {
    ...
    applyRealtimeEnvelope(qc(mock), { event: 'cash.closure.updated', payload: { businessDate: '2026-03-30', grossRevenueAmount: 0, realizedProfitAmount: 0 } })
  ...
  it('returns summaryNeedsRefresh when no kpi fields provided', () => {
    const mock = createQueryClientMock(liveSnapshot(), undefined, summarySnapshot())
    const result = applyRealtimeEnvelope(qc(mock), { event: 'cash.closure.updated', payload: { businessDate: '2026-03-30', status: 'CLOSED' } })
  ...
  it('ignores invalid nullable fields without breaking summary patch', () => {
    ...
    const result = applyRealtimeEnvelope(qc(mock), {
      event: 'cash.closure.updated',
      payload: { closureId: 'cl-1', status: 'CLOSED', expectedAmount: 140, grossRevenueAmount: 190, realizedProfitAmount: 60, countedAmount: 'invalid-number', differenceAmount: { nope: true }, openComandasCount: 1, pendingCashSessions: 0, businessDate: 12345 },
    })
