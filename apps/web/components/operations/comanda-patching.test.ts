...
describe('comanda.opened', () => {
  ...
  it('hydrates opened comanda with detailed items and payments from delta payload', () => {
    const mock = createQueryClientMock(liveSnapshot())
    applyRealtimeEnvelope(qc(mock), {
      event: 'comanda.opened',
      payload: {
        ...
        participantCount: 2,
        businessDate: '2026-03-30',
        items: [comandaItem({ id: 'item-1', productName: 'Coxinha', quantity: 2, unitPrice: 10, totalAmount: 20 })],
        payments: [
          {
            id: 'payment-1',
            method: 'PIX',
            ...
            note: 'Point',
            status: 'CONFIRMED',
            paidAt: '2026-03-30T10:05:00.000Z',
          },
        ],
      },
    })
    const comanda = mock.getLiveSnapshot().unassigned.comandas.find((entry) => entry.id === 'c-rich')
    expect(comanda).toBeTruthy()
    expect(comanda?.items).toHaveLength(1)
    expect(comanda?.payments).toEqual([expect.objectContaining({ id: 'payment-1', method: 'PIX', amount: 10 })])
    expect(comanda?.paidAmount).toBe(10)
    expect(comanda?.remainingAmount).toBe(20)
    expect(comanda?.paymentStatus).toBe('PARTIAL')
  })
...
describe('comanda.closed', () => {
  it('closes comanda and decrements openComandasCount', () => {
    ...
    applyRealtimeEnvelope(qc(mock), {
      ...
      payload: {
        ...
        totalAmount: 10,
        openedAt: '2026-03-30T10:00:00.000Z',
        items: [comandaItem({ id: 'item-1', productName: 'Coca-Cola', quantity: 2, unitPrice: 5, totalAmount: 10 })],
        businessDate: '2026-03-30',
        ...
        payments: [
          {
            ...
            status: 'CONFIRMED',
            paidAt: '2026-03-30T12:00:00.000Z',
          },
        ],
      },
    })
    ...
    expect(snap.unassigned.comandas[0]?.payments).toEqual([
      expect.objectContaining({ id: 'payment-1', method: 'CREDIT', amount: 10 }),
    ])
    expect(snap.unassigned.comandas[0]?.paidAmount).toBe(10)
    expect(snap.unassigned.comandas[0]?.remainingAmount).toBe(0)
    expect(snap.unassigned.comandas[0]?.paymentStatus).toBe('PAID')
  ...
  it('removes kitchen items for closed comanda', () => {
    ...
    if (!ks) {
      throw new Error('Expected kitchen snapshot to exist after realtime patch')
    }
  ...
  it('builds a printable closed comanda from delta payload when details are present', () => {
    const mock = createQueryClientMock(liveSnapshot(), kitchenSnapshot())
    const result = applyRealtimeEnvelope(qc(mock), {
      event: 'comanda.closed',
      payload: {
        comandaId: 'c-terminal',
        mesaId: 'mesa-9',
        mesaLabel: 'Mesa 9',
        status: 'CLOSED',
        subtotal: 22,
        ...
        totalAmount: 22,
        customerName: 'Ana',
        notes: 'Pagamento no terminal',
        participantCount: 1,
        openedAt: '2026-03-30T11:50:00.000Z',
        closedAt: '2026-03-30T12:00:00.000Z',
        businessDate: '2026-03-30',
        items: [comandaItem({ id: 'item-1', productName: 'Suco', quantity: 2, unitPrice: 11, totalAmount: 22 })],
        payments: [
          {
            id: 'payment-2',
            method: 'PIX',
            amount: 22,
            note: 'Mercado Pago Point - PIX. Transacao MP payment-provider-2',
            status: 'CONFIRMED',
            paidAt: '2026-03-30T12:00:00.000Z',
          },
        ],
    ...
    expect(result.livePatched).toBe(true)
    const comanda = mock.getLiveSnapshot().unassigned.comandas.find((entry) => entry.id === 'c-terminal')
    expect(comanda).toBeTruthy()
    expect(comanda?.status).toBe('CLOSED')
    expect(comanda?.items).toHaveLength(1)
    expect(comanda?.payments).toEqual([expect.objectContaining({ id: 'payment-2', method: 'PIX' })])
    expect(comanda?.paidAmount).toBe(22)
    expect(comanda?.paymentStatus).toBe('PAID')
  })
})
