...
describe('ProductThumb', () => {
  ...
  it('usa iniciais para bebida embalada sem foto real', () => {
    ...
    expect(screen.getByText('CL').closest('[data-product-visual-source="initials"]')).toBeInTheDocument()
  })

  it('em modo real-only nao cai em fallback ilustrativo local', () => {
    render(
      <ProductThumb
        product={{
          name: 'Coca-Cola Lata',
    ...
    expect(screen.queryByAltText('Packshot de Coca-Cola Lata')).not.toBeInTheDocument()
    expect(screen.getByText('CL').closest('[data-product-visual-source="initials"]')).toBeInTheDocument()
  })
