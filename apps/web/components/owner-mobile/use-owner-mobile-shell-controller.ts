...
import {
  resolveOwnerSettingsSection,
  resolveOwnerTab,
  useOwnerMobileShellUrlSync,
} from './use-owner-mobile-shell-url-sync'
...
function useRealtimeRefresh(
  ...
  return {
    isOffline,
    realtimeStatus,
    ...usePullToRefresh({ enabled: pullToRefreshEnabled, onRefresh: handlePullRefresh }),
  }
}
...
export function useOwnerMobileShellController(currentUser: OwnerCurrentUser | null) {
  ...
  const pullToRefreshEnabled = !(shellState.activeTab === 'pdv' && shellState.pendingAction)
  ...
  const queries = useOwnerMobileShellQueries(
    currentUser,
    {
  ...
  useOwnerMobileShellUrlSync({ panelParam, setActiveSettingsSection, setActiveTab, tabParam, viewParam })
