...
describe('useOwnerMobileShellMutations', () => {
  ...
  it('envia intent de pagamento na maquininha sem mutar o snapshot antes do webhook', async () => {
    const snapshot = buildOperationsSnapshot({
      employees: [],
      unassigned: {
        comandas: [
          buildComanda({
            ...
            tableLabel: '1',
            totalAmount: 80,
            items: [{ id: 'item-1', productName: 'Base', quantity: 1, unitPrice: 80 }],
          }),
        ],
      },
    })
    ...
    vi.mocked(api.createComandaTerminalPaymentIntent).mockResolvedValue({
      terminalPaymentIntent: {
        ...
        externalReference: 'comanda:c-1:intent:intent-1',
    ...
    await act(async () => {
      await result.current.createComandaTerminalPaymentIntentMutation.mutateAsync({
        amount: 80,
        comandaId: 'c-1',
        method: 'CREDIT',
      })
    ...
    expect(api.createComandaTerminalPaymentIntent).toHaveBeenCalledWith('c-1', {
      amount: 80,
      method: 'CREDIT',
      replacePending: true,
    })
    expect(queryClient.getQueryData(OPERATIONS_LIVE_COMPACT_QUERY_KEY)).toBe(snapshot)
  })
})
