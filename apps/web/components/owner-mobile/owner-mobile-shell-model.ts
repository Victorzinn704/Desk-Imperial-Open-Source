...
export function buildOwnerMobileShellControllerValue(args: ControllerBuildArgs) {
  return {
    ...
    isBusy: resolveOwnerMobileMutationPending(args.mutations),
    router: args.router,
    navigateSettingsSection: args.navigateSettingsSection,
  }
}

function resolveOwnerMobileMutationPending(mutations: ReturnType<typeof useOwnerMobileShellMutations>) {
  return (
    mutations.openComandaMutation.isPending ||
    mutations.addComandaItemMutation.isPending ||
    mutations.addComandaItemsMutation.isPending ||
    mutations.replaceComandaMutation.isPending ||
    mutations.updateComandaStatusMutation.isPending ||
    mutations.closeComandaMutation.isPending ||
    mutations.createComandaPaymentMutation.isPending ||
    mutations.createComandaTerminalPaymentIntentMutation.isPending ||
    mutations.openCashSessionMutation.isPending
  )
}
