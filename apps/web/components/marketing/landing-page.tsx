...
type HeroTitleProps = {
  lineShifts: Array<ReturnType<typeof useSpring>>
  onPointerLeave: () => void
  onPointerMove: (event: ReactMouseEvent<HTMLDivElement>) => void
}

function HeroTitle({ lineShifts, onPointerLeave, onPointerMove }: HeroTitleProps) {
  return (
    <motion.div
      animate="visible"
...
function HeroPrimaryColumn({ rotateX, rotateY, lineShifts, onPointerLeave, onPointerMove }: HeroPrimaryColumnProps) {
  return (
    <motion.div
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      <div style={{ perspective: 1200 }}>
        <motion.div style={{ rotateX, rotateY }}>
          <HeroTitle lineShifts={lineShifts} onPointerLeave={onPointerLeave} onPointerMove={onPointerMove} />
        </motion.div>
      </div>
      ...
      <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--text-soft)]">
        Anote o pedido, veja quanto entrou, pague sua equipe certo e descubra de onde vêm seus clientes. Tudo no mesmo
        sistema, sem precisar de planilha.
      </p>
...
function FoundationSection() {
  return (
    <section className="relative z-10" id="fundacao">
      ...
      {foundationFeatures.map(({ side, tag, title, body, stat, icon: Icon }, i) => (
        <motion.div
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-4 text-base leading-7 text-[var(--text-soft)]">
                  Crie sua conta, entre no painel e veja tudo funcionando de imediato. Não tem tutorial obrigatório nem
            ...
            <div className="fcard-wrap" key={group.title}>
              <div className="fcard-obj">
                ...
                <div className="fcard-face" style={{ animationDelay: `${gi * 0.5}s` }}>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--accent)]">{group.title}</p>
