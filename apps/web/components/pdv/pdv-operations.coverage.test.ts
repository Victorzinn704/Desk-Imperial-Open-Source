 ...
 *   - buildPdvMesas() fallback only from active comandas (no invented free tables)
...
describe('pdv-operations — coverage boost', () => {
  ...
  it('buildPdvMesas retorna array vazio quando snapshot e undefined', () => {
    expect(buildPdvMesas(undefined)).toEqual([])
  })

  it('buildPdvMesas sem mesas do banco nao inventa mesas livres', () => {
    const snapshot = buildOperationsSnapshot({ employees: [], mesas: [] })
    expect(buildPdvMesas(snapshot)).toEqual([])
  })

  it('buildPdvMesas deriva ocupacao a partir de comandas ativas quando nao existe mapa de mesas', () => {
    const snapshot = buildOperationsSnapshot({
      employees: [
        {
          employeeId: 'emp-1',
          displayName: 'Marina',
          comandas: [{ id: 'c-open', tableLabel: 'Mesa 7', status: 'OPEN', currentEmployeeId: 'emp-1' }],
        },
      ],
      mesas: [],
    })
