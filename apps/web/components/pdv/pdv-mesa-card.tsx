...
export function PdvMesaCard({
  ...
  return (
    ...
    >
      {/* Delete button */}
      <button
        className="absolute right-3 top-3 flex size-6 items-center justify-center rounded-full text-[var(--text-muted)] opacity-0 transition hover:text-white group-hover:opacity-100 hover:opacity-100 hover:bg-[rgba(255,255,255,0.06)]"
        type="button"
      ...
      {mesa.status === 'ocupada' && comanda && (
        <div
          className="mt-3 rounded-xl border border-[rgba(251,146,60,0.18)] bg-[rgba(251,146,60,0.05)] p-3"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-[var(--text-soft)]">
              <ShoppingBag className="size-3.5" />
              <span className="text-[11px]">{comanda.itens.length} {comanda.itens.length === 1 ? 'item' : 'itens'}</span>
            </div>
            <div className="flex items-center gap-1.5 text-[var(--text-soft)]">
          ...
          {comanda.clienteNome && (
            <p className="mt-1.5 text-[11px] font-medium text-white truncate">{comanda.clienteNome}</p>
          )}
          <p className="mt-1 text-sm font-bold text-[#fb923c]">
