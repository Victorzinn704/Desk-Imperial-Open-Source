...
export function usePdvBoardIntent({
  ...
  useEffect(() => {
    ...
    const mesaFromIntent = resolveMesaFromIntent({ mesaIntent, mesas, mesasById })
...
function resolveMesaFromIntent({
  mesaIntent,
  mesas,
  mesasById,
}: Readonly<{
  mesaIntent: PdvMesaIntent
  mesas: readonly Mesa[]
  mesasById: ReadonlyMap<string, Mesa>
}>) {
  return (
    mesasById.get(mesaIntent.mesaId) ??
    resolveSelectedMesa([...mesas], mesaIntent.mesaLabel) ??
    buildTrustedMesaFromIntent(mesaIntent)
  )
}

function buildTrustedMesaFromIntent(mesaIntent: PdvMesaIntent) {
  if (normalizeTableLabel(mesaIntent.mesaId) === normalizeTableLabel(mesaIntent.mesaLabel)) {
    return null
  }

  return {
    id: mesaIntent.mesaId,
    numero: mesaIntent.mesaLabel,
    capacidade: 4,
