...
function useStaffMobileShellModules(
  ...
  const pullToRefreshEnabled = !(shellState.activeTab === 'pedido' && shellState.pendingAction)
  const realtime = useRealtimeRefresh(Boolean(currentUser), pullToRefreshEnabled, currentUser?.userId ?? null)
  const queries = useStaffMobileShellQueries(
    currentUser,
    {
