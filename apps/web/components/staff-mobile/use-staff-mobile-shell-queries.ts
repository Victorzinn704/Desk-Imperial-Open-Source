...
const STAFF_KITCHEN_WARM_TABS = new Set<StaffMobileShellQueryScope['activeTab']>(['mesas', 'cozinha', 'pedido'])
...
function useStaffOperationsQuery(enabled: boolean, realtimeStatus: RealtimeStatus) {
  return useQuery({
...
function useStaffKitchenQuery(enabled: boolean, realtimeStatus: RealtimeStatus) {
  return useQuery({
...
export function buildStaffQueryEnablement(enabled: boolean, scope: StaffMobileShellQueryScope) {
  ...
  return {
    kitchen: enabled && shouldKeepStaffKitchenWarm(scope),
    operations: enabled,
...
function shouldKeepStaffKitchenWarm(scope: StaffMobileShellQueryScope) {
  if (STAFF_KITCHEN_WARM_TABS.has(scope.activeTab)) {
    return true
  }
...
export function useStaffMobileShellQueries(
  currentUser: StaffMobileCurrentUser,
  scope: StaffMobileShellQueryScope,
  realtimeStatus: RealtimeStatus,
) {
  const enabled = Boolean(currentUser)
  const queryEnablement = buildStaffQueryEnablement(enabled, scope)
  const operationsQuery = useStaffOperationsQuery(queryEnablement.operations, realtimeStatus)
  const kitchenQuery = useStaffKitchenQuery(queryEnablement.kitchen, realtimeStatus)
