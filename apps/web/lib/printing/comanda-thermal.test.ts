...
describe('buildThermalComandaTicket', () => {
  it('includes DESK IMPERIAL header', () => {
    ...
    expect(ticket).toContain('DOCUMENTO NAO FISCAL')
  ...
  it('prints payment section with transaction note when payments exist', () => {
    const ticket = buildThermalComandaTicket(
      makeComanda({
        payments: [
          {
            ...
            paidAtIso: '2026-04-09T14:35:00.000Z',
          },
        ],
      }),
    )

    expect(ticket).toContain('PAGAMENTOS')
    expect(ticket).toContain('CREDITO')
    expect(ticket).toContain('Transacao MP')
    expect(ticket).toContain('payment-provider-1')
    expect(ticket).toContain('PAGO EM')
    expect(ticket).toContain('FECHADA')
    expect(ticket).toContain('VIA CLIENTE')
  })
})
