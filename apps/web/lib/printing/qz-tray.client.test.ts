...
describe('qz-tray client', () => {
  ...
  it('strips any pasted port (not only QZ ports) so localhost:3000 normalises to localhost', async () => {
    const { listQzTrayPrinters, setQzHost } = await import('./qz-tray.client')

    setQzHost('localhost:3000')
    await listQzTrayPrinters()
  ...
  it('self-heals stale localStorage hosts when getQzHost is read', async () => {
    window.localStorage.setItem('desk-imperial.qz-host', 'localhost:3000/garbage')
    const { getQzHost } = await import('./qz-tray.client')

    expect(getQzHost()).toBe('localhost')
    expect(window.localStorage.getItem('desk-imperial.qz-host')).toBe('localhost')
  })

  it('translates "blocked by client" into a user-actionable adblocker hint', async () => {
    qzMock.websocket.connect.mockRejectedValue(new Error('net::ERR_BLOCKED_BY_CLIENT'))
    const { listQzTrayPrinters } = await import('./qz-tray.client')

    await expect(listQzTrayPrinters()).rejects.toThrow(/extensao de bloqueio|uBlock|AdGuard/i)
  })
