...
export const OPERATIONS_HOT_PATH_RECONCILE_DELAY_MS = 250
export const OPERATIONS_COMMERCIAL_RECONCILE_DELAY_MS = 900
...
export function scheduleOperationsWorkspaceReconcile(
  ...
  const { delayMs: requestedDelayMs, ...invalidateOptions } = options ?? {}
  const normalizedOptions = normalizeInvalidateOperationsWorkspaceOptions(invalidateOptions)
  const delayMs = resolveReconcileDelayMs(requestedDelayMs, normalizedOptions)
...
function resolveReconcileDelayMs(
  requestedDelayMs: number | undefined,
  options: NormalizedInvalidateOperationsWorkspaceOptions,
) {
  if (typeof requestedDelayMs === 'number') {
    return requestedDelayMs
  }

  return options.includeOrders || options.includeFinance
