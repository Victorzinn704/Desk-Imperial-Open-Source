...
import { createMobilePerformanceReporter, summarizeRealtimeEvents } from './operations-mobile-perf-report.mjs'

const APP_BASE_URL = process.env.DESK_SMOKE_APP_BASE_URL ?? 'http://localhost:3100'
...
const SMOKE_KITCHEN_PRODUCT_NAME = process.env.DESK_SMOKE_KITCHEN_PRODUCT_NAME ?? 'Smoke Cozinha Performance'
const perfReporter = createMobilePerformanceReporter({
...
async function main() {
  ...
  const kitchenProduct = await resolveKitchenProduct(ownerApi, products)
  const anyActiveProduct = products.find(isAvailableProduct) ?? kitchenProduct
  ...
  try {
    ...
    const result = perfReporter.buildResult(summary)
    await perfReporter.writeIfRequested(result)
    perfReporter.print(result)
    perfReporter.assertBudget(result)
  } finally {
...
async function resolveKitchenProduct(ownerApi, products) {
  const kitchenProduct = products.find(isAvailableKitchenProduct)
  if (kitchenProduct) {
    return kitchenProduct
  }

  if (!canPrepareSmokeFixtures()) {
    throw new Error('Nenhum produto de cozinha disponivel para o smoke. Habilite DESK_SMOKE_ALLOW_FIXTURES=true.')
  }

  return ensureSmokeKitchenProduct(ownerApi, products)
}

async function ensureSmokeKitchenProduct(ownerApi, products) {
  const existingFixture = products.find((product) => product.name === SMOKE_KITCHEN_PRODUCT_NAME)
  if (existingFixture) {
    return activateSmokeKitchenProduct(ownerApi, existingFixture)
  }

  const created = await ownerApi.request('/products', {
    method: 'POST',
    body: JSON.stringify(buildSmokeKitchenProductPayload()),
  })
  return created.product
}

async function activateSmokeKitchenProduct(ownerApi, product) {
  const updated = await ownerApi.request(`/products/${product.id}`, {
    method: 'PATCH',
    body: JSON.stringify({
      active: true,
      category: 'Porções',
      requiresKitchen: true,
      stock: Math.max(product.stock ?? 0, 50),
    }),
  })
  return updated.product
}

function buildSmokeKitchenProductPayload() {
  return {
    name: SMOKE_KITCHEN_PRODUCT_NAME,
    brand: 'Desk Imperial',
    category: 'Porções',
    packagingClass: 'Unidade de preparo',
    measurementUnit: 'un',
    measurementValue: 1,
    unitsPerPackage: 1,
    description: 'Produto tecnico criado pelo smoke de performance local para validar fila de cozinha.',
    unitCost: 4,
    unitPrice: 12,
    currency: 'BRL',
    stock: 50,
...
function canPrepareSmokeFixtures() {
  const configuredValue = process.env.DESK_SMOKE_ALLOW_FIXTURES?.trim().toLowerCase()
...
function isAvailableKitchenProduct(product) {
  return isAvailableProduct(product) && product.requiresKitchen
}
...
async function bootstrapAuthenticatedPage(context, page, session, path, readySelector) {
  await context.addCookies(buildBrowserSessionCookies(session))
...
function buildBrowserSessionCookies(session) {
  const origins = resolveBrowserSessionOrigins()
...
function resolveBrowserSessionOrigins() {
  const origins = [new URL(API_BASE_URL).origin, new URL(APP_BASE_URL).origin]
  if (usesLocalLoopback()) {
    origins.push('http://localhost:3000', 'http://localhost:4000', 'http://127.0.0.1:3000', 'http://127.0.0.1:4000')
  }

  return [...new Set(origins)]
}

function usesLocalLoopback() {
  return [new URL(API_BASE_URL).hostname, new URL(APP_BASE_URL).hostname].some((hostname) =>
    ['localhost', '127.0.0.1'].includes(hostname.toLowerCase()),
  )
}
...
function summarizePerfEvents(events) {
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case 'mutation-finished':
        ...
        break
      case 'realtime-envelope-processed':
        realtimeEvents.push(event)
        break
      case 'realtime-envelope-dropped':
        dropped += 1
        break
      case 'realtime-envelope-out-of-order':
        outOfOrder += 1
        break
      case 'reconnect-refresh':
  ...
  return {
    ...
    events,
    mutations,
    realtime: summarizeRealtimeEvents({ dropped, outOfOrder, realtimeEvents, reconnects }),
  }
}
