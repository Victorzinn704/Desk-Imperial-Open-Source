...
describe('validateEnvironment', () => {
  ...
  it('exige secret de webhook quando o bot Telegram está configurado em produção', () => {
    expect(() =>
      validateEnvironment({
        NODE_ENV: 'production',
        DATABASE_URL: 'postgres://user:pass@localhost:5432/desk',
    ...
    ).toThrow(expect.objectContaining({ message: expect.stringContaining('TELEGRAM_WEBHOOK_SECRET é obrigatório') }))
  })

  it('exige credenciais operacionais quando Mercado Pago Point esta parcialmente configurado', () => {
    expect(() =>
      validateEnvironment({
        NODE_ENV: 'production',
        DATABASE_URL: 'postgres://user:pass@localhost:5432/desk',
    ...
    ).toThrow(expect.objectContaining({ message: expect.stringContaining('MERCADO_PAGO_POINT_TERMINAL_ID') }))
  })

  it('aceita Mercado Pago Point completo em producao', () => {
    const env = validateEnvironment({
      NODE_ENV: 'production',
      DATABASE_URL: 'postgres://user:pass@localhost:5432/desk',
      COOKIE_SECRET: '1234567890123456',
      ...
      REDIS_PRIVATE_URL: 'redis://localhost:6379',
      MERCADO_PAGO_ACCESS_TOKEN: 'APP_USR-token',
      MERCADO_PAGO_POINT_TERMINAL_ID: 'NEWLAND_N950__N950NCB801293324',
      MERCADO_PAGO_WEBHOOK_SECRET: 'mercado-pago-webhook-secret',
      MERCADO_PAGO_ORDERS_URL: 'https://api.mercadopago.com/v1/orders',
    })

    expect(env.MERCADO_PAGO_POINT_TERMINAL_ID).toContain('NEWLAND_N950')
  })
