...
describe('NotificationsController (telegram)', () => {
  ...
  beforeEach(() => {
    ...
    const telegramRuntime = {
      ...
      enqueueInbound: jest.fn(async () => true),
    }
  ...
  it('rejeita webhook quando o secret esperado não está configurado', async () => {
    telegramAdapter.getWebhookSecret.mockReturnValueOnce(null as never)
    const request = {
      body: { update_id: 1 },
      headers: {},
      ip: undefined,
  ...
  it('aceita webhook válido e encaminha o update para o bot service', async () => {
    ...
    // No fallback inline (runtime sem Redis), o bot service ainda recebe direto. Em produção,
    // com runtime habilitado, o controller enfileira e o worker chama handleWebhookUpdate.
    ...
    expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ ok: true, accepted: true }))
  })
})
