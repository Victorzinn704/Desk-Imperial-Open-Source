...
describe('OperationsRealtimeGateway', () => {
  ...
  it('revokeSessionsCrossPod invalida o cache de auth para sessão revogada', async () => {
    const { gateway, authService } = makeGateway()
    ...
    const makeSocket = () =>
      ({
        id: 'socket-cache',
        handshake: { auth: { token: 'Bearer revokable-token' } },
        data: {},
      }) as any

    await gateway.authenticateConnection(makeSocket())
    await gateway.revokeSessionsCrossPod(['session-revoked'])
    const result = await gateway.authenticateConnection(makeSocket())

    expect(authService.validateSessionToken).toHaveBeenCalledTimes(2)
    expect(result.auth.sessionId).toBe('session-fresh')
  })
