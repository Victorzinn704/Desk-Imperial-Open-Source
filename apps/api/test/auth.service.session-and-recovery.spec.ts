...
import { sendPasswordChangedNotice } from '../src/modules/auth/auth-password-delivery.utils'
...
function createSetup(options: SetupOptions = {}) {
  ...
  const registrationPolicy = new AuthRegistrationPolicyService(config as unknown as ConfigService)
...
describe('AuthService session and recovery flows', () => {
  ...
  const genericEmailVerificationResponse = {
    success: true,
    message: 'Se o email estiver cadastrado e pendente de confirmacao, enviaremos um novo codigo em instantes.',
  }

  const expectMissingUserError = async (
    invoke: (service: AuthService) => Promise<unknown>,
    rateLimitKey: 'recordEmailVerificationCodeAttempt' | 'recordPasswordResetCodeAttempt',
  ) => {
    const { service, prisma, rateLimit } = createSetup()
    prisma.user.findUnique.mockResolvedValue(null)

    await expect(invoke(service)).rejects.toThrow('Email invalido ou nao cadastrado.')

    expect(rateLimit[rateLimitKey] as jest.Mock).toHaveBeenCalledTimes(2)
  }
  ...
  it.each([
    {
      title: 'requestEmailVerification retorna mensagem generica quando email ja esta verificado',
      user: {
        id: 'user-1',
        fullName: 'Owner',
        email: 'owner@empresa.com',
    ...
    {
      title: 'requestEmailVerification retorna mensagem generica quando email nao existe',
      user: null,
      email: 'naoexiste@empresa.com',
    },
  ])('$title', async ({ user, email }) => {
    const { service, prisma, audit } = createSetup()
    prisma.user.findUnique.mockResolvedValue(user)

    const result = await service.requestEmailVerification({ email }, makeRequestContext())

    expect(result).toEqual(genericEmailVerificationResponse)
  ...
  it('verifyEmail incrementa tentativas e falha para email inexistente', async () => {
    await expectMissingUserError(
      ...
      'recordEmailVerificationCodeAttempt',
    )
  })
  ...
  it('resetPassword falha para email inexistente e registra tentativas', async () => {
    await expectMissingUserError(
      ...
      'recordPasswordResetCodeAttempt',
    )
  })
  ...
  it('sendPasswordChangedNotice envia notificacao e audita entrega', async () => {
    const { mailer, audit } = createSetup()

    await sendPasswordChangedNotice({
      dependencies: {
        auditLogService: audit as never,
        logger: { warn: jest.fn() } as never,
        mailerService: mailer as never,
      },
      user: {
        id: 'user-1',
        email: 'owner@empresa.com',
      ...
      changedAt: new Date('2026-04-03T00:00:00.000Z'),
    })
