...
import { sanitizeProductCatalogImageUrl } from '../src/modules/products/products-catalog.util'
...
type BackfillMode = 'missing-only' | 'refresh-pexels'
...
async function main() {
  ...
  if (!PEXELS_API_KEY) {
    throw new Error('PEXELS_API_KEY ainda não foi configurada. Rode o script PowerShell de setup antes do backfill.')
  }
  ...
  const sampleChanges: Array<{ id: string; name: string; previousSource: string | null; nextImageUrl: string }> = []
...
function parseOptions(argv: string[]): ScriptOptions {
  ...
  let mode: BackfillMode = 'missing-only'
...
function shouldProcessProduct(product: ProductImageCandidate, mode: BackfillMode) {
...
function buildProductImageSearchQuery(product: ProductImageCandidate) {
...
function isPackagedBeverageLike(product: ProductImageCandidate) {
