...
const OPTIONAL_MERCADO_PAGO_URL_KEYS = ['MERCADO_PAGO_ORDERS_URL'] as const
...
export function validateEnvironment(config: EnvShape) {
  ...
  validateUrlGroup(env, OPTIONAL_MERCADO_PAGO_URL_KEYS, issues)
  ...
  validateMercadoPagoPointConfig(env, issues)
...
function validateMercadoPagoPointConfig(env: EnvShape, issues: string[]) {
  if (!isMercadoPagoPointRuntimeEnabled(env)) {
    return
  }

  validateRequiredSecret(env.MERCADO_PAGO_ACCESS_TOKEN, 'MERCADO_PAGO_ACCESS_TOKEN', issues)
  validateRequiredSecret(env.MERCADO_PAGO_POINT_TERMINAL_ID, 'MERCADO_PAGO_POINT_TERMINAL_ID', issues)
  validateRequiredSecret(env.MERCADO_PAGO_WEBHOOK_SECRET, 'MERCADO_PAGO_WEBHOOK_SECRET', issues)
  validateSecretLength(env.MERCADO_PAGO_WEBHOOK_SECRET, 24, 'MERCADO_PAGO_WEBHOOK_SECRET', issues)
}
...
function validateProductionRequirements(env: EnvShape, nodeEnv: string, issues: string[]) {
  ...
  if (isTelegramBotRuntimeEnabled(env)) {
    ...
    if (!env.TELEGRAM_WEBHOOK_SECRET?.trim()) {
      issues.push('TELEGRAM_WEBHOOK_SECRET é obrigatório em produção quando o bot Telegram está habilitado.')
    }
  }
}

function isTelegramBotRuntimeEnabled(env: EnvShape) {
  return Boolean(env.TELEGRAM_BOT_TOKEN?.trim()) && env.TELEGRAM_BOT_ENABLED !== 'false'
}

function isMercadoPagoPointRuntimeEnabled(env: EnvShape) {
  return Boolean(
    env.MERCADO_PAGO_ACCESS_TOKEN?.trim() ||
    env.MERCADO_PAGO_POINT_TERMINAL_ID?.trim() ||
