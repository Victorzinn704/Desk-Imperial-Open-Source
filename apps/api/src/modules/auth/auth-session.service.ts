...
import {
  ...
  buildSessionAuthContext,
  isCachedAuthSessionFresh,
  resolveSessionRejectionReason,
  shouldRefreshSessionLastSeen,
} from './auth-session-validation.utils'
import { authSessionEmployeeSelect, authSessionUserSelect, authSessionWorkspaceOwnerSelect } from './auth-shared.util'
...
type NegativeAuthSessionCacheEntry = {
  tokenHash: string
  reason: AuthSessionRejectionReason
  negative: true
}
...
export class AuthSessionService {
  ...
  async validateSessionToken(rawToken: string): Promise<AuthContext | null> {
    ...
    const cachedAuth = await this.resolveCachedAuthSession(tokenHash)

    if (cachedAuth !== undefined) {
      return cachedAuth
    }

    if (await this.hasCachedNegativeSession(tokenHash)) {
      return null
    }
    ...
    const session = await this.loadSessionByTokenHash(tokenHash)
    ...
    const rejectionReason = resolveSessionRejectionReason(session)
    if (rejectionReason) {
      await this.cacheNegativeSessionToken(tokenHash, rejectionReason)
      await this.demoAccessService.closeGrantForSession(session.id)
      return null
    }

    if (shouldRefreshSessionLastSeen(session)) {
    ...
    const auth = buildSessionAuthContext({
      session,
      buildEvaluationAccess: (email, expiresAt) => this.demoAccessService.buildEvaluationAccess(email, expiresAt),
    })
  ...
  private async resolveCachedAuthSession(tokenHash: string): Promise<AuthContext | null | undefined> {
    ...
    if (isCachedAuthSessionFresh(cached.expiresAt)) {
  ...
  private async hasCachedNegativeSession(tokenHash: string) {
  ...
  private async loadSessionByTokenHash(tokenHash: string): Promise<AuthSessionRecord | null> {
