...
export async function buildCashMovementResponse(input: {
  helpers: OperationsHelpersService
  workspaceOwnerUserId: string
  businessDate: Date
  ...
  return {
    movement: toCashMovementRecord(input.movement),
    cashSession: toCashSessionRecord(input.session),
    ...(await buildOptionalOperationsSnapshot(
      input.helpers,
