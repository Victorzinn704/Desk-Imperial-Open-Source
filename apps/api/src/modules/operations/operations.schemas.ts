import {
  ...
  paymentTerminalIntentRecordSchema,
...
export const createComandaTerminalPaymentIntentBodySchema = z
  .object({
    amount: createOptionalMoneySchema(0.01),
    method: z.enum(['PIX', 'DEBIT', 'CREDIT']),
    note: z.string().max(160).optional(),
    replacePending: z.preprocess(coerceLooseOptionalBoolean, z.boolean().optional()),
    terminalId: z.string().trim().min(1).max(120).optional(),
  })
  .strict()
export type CreateComandaTerminalPaymentIntentDto = z.infer<typeof createComandaTerminalPaymentIntentBodySchema>
...
export const comandaTerminalPaymentIntentResponseSchema = z
  .object({ terminalPaymentIntent: paymentTerminalIntentRecordSchema })
  .strict()
