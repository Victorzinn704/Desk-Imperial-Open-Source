...
export function publishCashRealtime(input: {
  ...
  input.operationsRealtimeService.publishCashUpdated(
    input.auth,
    {
      ...buildCashUpdatedPayload(input.session),
      ...(input.businessDate ? { businessDate: formatBusinessDateKey(input.businessDate) } : {}),
      cashSession: toRealtimeCashSessionRecord(input.session),
    },
    input.instrumentation,
  )
  publishCashClosureRealtime(input.operationsRealtimeService, input.auth, input.closure)
}
