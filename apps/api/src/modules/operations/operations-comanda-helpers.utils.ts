...
function resolveLinkedDraftItem(
  ...
  const product = productById.get(requireDraftProductId(item))
  if (!product) {
    throw new NotFoundException('Produto nao encontrado para esta conta.')
  }
  return {
...
function resolveManualDraftItem(item: ComandaDraftItemDto) {
  ...
  if (!productName) {
    throw new BadRequestException('Informe o nome do item quando ele nao estiver vinculado ao catalogo.')
  }
  ...
  return {
    ...
    productName,
    requiresKitchen: isKitchenCategory(productName),
    unitPrice: roundCurrency(item.unitPrice),
  }
}

function requireDraftProductId(item: ComandaDraftItemDto) {
  if (!item.productId) {
    throw new BadRequestException('Produto nao encontrado para esta conta.')
  }

  return item.productId
}

function hasProductId<T extends { productId?: string | null }>(item: T): item is T & { productId: string } {
...
export async function ensureOrderForClosedComanda(
  ...
  const stockByProduct = buildProductConsumptionMap(
    comanda.items.filter(hasProductId).map((item) => ({
