...
function buildPreparedComandaItem(
  ...
  const requiresKitchen = resolveRequiresKitchen(product, productName)
...
function resolveRequiresKitchen(product: ComandaItemProduct | null | undefined, productName: string) {
  if (product) {
    return product.requiresKitchen || isKitchenCategory(product.category)
  }

  return isKitchenCategory(productName)
}
