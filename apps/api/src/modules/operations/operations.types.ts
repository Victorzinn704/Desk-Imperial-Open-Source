...
export function toRealtimeComandaRecord(comanda: RealtimeComandaLike): ComandaRecord {
  ...
  return {
    ...
    status: resolveRealtimeComandaStatus(comanda),
    subtotalAmount: toNumberOrNull(comanda.subtotalAmount) ?? totalAmount,
    discountAmount: toNumberOrNull(comanda.discountAmount) ?? 0,
    serviceFeeAmount: toNumberOrNull(comanda.serviceFeeAmount) ?? 0,
    totalAmount,
    notes: comanda.notes ?? null,
    openedAt: resolveRealtimeComandaOpenedAt(comanda, now),
    closedAt: comanda.closedAt?.toISOString() ?? null,
    items: mapRealtimeComandaItems(comanda),
  }
}

function resolveRealtimeComandaStatus(comanda: RealtimeComandaLike): ComandaRecord['status'] {
  return comanda.status ?? (comanda.closedAt ? 'CLOSED' : 'OPEN')
}

function resolveRealtimeComandaOpenedAt(comanda: RealtimeComandaLike, now: Date) {
  return (comanda.openedAt ?? comanda.closedAt ?? now).toISOString()
}

function mapRealtimeComandaItems(comanda: RealtimeComandaLike): ComandaRecord['items'] {
  return (
