  ...
  for (const row of rows) {
    ...
    current.inventoryCostValue = roundCurrency(
      current.inventoryCostValue +
        convertFinanceAmount({ amount: toNumber(row._sum.lineCost), currency: row.currency, options }),
    )
    current.inventorySalesValue = roundCurrency(
      current.inventorySalesValue +
        convertFinanceAmount({ amount: toNumber(row._sum.lineRevenue), currency: row.currency, options }),
    )
    current.potentialProfit = roundCurrency(
      current.potentialProfit +
        convertFinanceAmount({ amount: toNumber(row._sum.lineProfit), currency: row.currency, options }),
    )
...
export function buildRecentOrders(orders: FinanceRecentOrder[], options: FinanceAggregationOptions) {
  return orders.map((order) => ({
    ...
    status: order.status,
    totalRevenue: convertFinanceAmount({ amount: toNumber(order.totalRevenue), currency: order.currency, options }),
    totalProfit: convertFinanceAmount({ amount: toNumber(order.totalProfit), currency: order.currency, options }),
