...
export class TelegramLinkService {
  ...
  async getHealth(): Promise<TelegramHealthResponse> {
    const runtime = await this.collectRuntimeSnapshot()
  ...
  private async collectRuntimeSnapshot() {
    const enabled = this.telegramRuntime.isReady()
    const [queueDepth, lastMinute] = await Promise.all([
      this.telegramRuntime.getQueueDepths(),
      this.telegramRuntime.readMetricsLastMinute(),
    ])
    return { enabled, queueDepth, lastMinute }
  }
