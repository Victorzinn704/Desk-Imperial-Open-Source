...
export class TelegramAdapter {
  ...
  buildDefaultCommands(): TelegramBotCommand[] {
    return [
      ...
      { command: 'fechamento', description: 'Fluxo guiado de fechamento (sangria + obs.)' },
      { command: 'cancelar', description: 'Cancela um fluxo guiado em andamento' },
  ...
  async setWebhook(url: string, secretToken: string | null) {
    const payload: { url: string; allowed_updates: string[]; drop_pending_updates: boolean; secret_token?: string } = {
      url,
    ...
    return this.getApi().setWebhook(url, payload as never)
  }

  async deleteWebhook(dropPendingUpdates = false) {
    return this.getApi().deleteWebhook({ drop_pending_updates: dropPendingUpdates })
  }
