...
export function calculateEffectiveUnitCost(
  ...
  if (comboComponents.length === 0) {
    return roundCurrency(
      convertProductCost({
        amount: toNumber(product.unitCost),
  ...
  return roundCurrency(
    comboComponents.reduce(
      (total, component) =>
        total +
        convertProductCost({
          amount: toNumber(component.componentProduct.unitCost) * component.totalUnits,
          currency: component.componentProduct.currency,
          options,
        }),
      0,
    ),
