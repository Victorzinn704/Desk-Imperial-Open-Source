...
export function calculateEffectiveUnitCost(
  ...
  if (comboComponents.length === 0) {
    return roundCurrency(
      options.currencyService.convert(toNumber(product.unitCost), product.currency, options.displayCurrency, options.snapshot),
    )
  }
...
export function calculateRawEffectiveUnitCost(product: ComboAwareInventoryProductLike) {
  ...
  return roundCurrency(
    comboComponents.reduce((total, component) => total + toNumber(component.componentProduct.unitCost) * component.totalUnits, 0),
  )
}
