...
├── mailer/                # Envio de email transacional (Brevo) usado por auth e notifications
...
O ciclo de vida de comanda esta dividido em servicos especializados (cada um com responsabilidade unica):

- `comanda.service.ts` — fachada do dominio
- `comanda-command-facade.service.ts` — orquestracao de comandos
- `comanda-open.service.ts` / `comanda-lifecycle.service.ts` / `comanda-settlement.service.ts` — abrir, transicionar e fechar
- `comanda-items.service.ts` / `comanda-items-dispatch.service.ts` — itens e despacho
- `comanda-kitchen.service.ts` / `comanda-kitchen-dispatch.service.ts` — cozinha e fila
- `comanda-query.service.ts` — leitura
- `comanda-realtime-publisher.service.ts` — emissao de envelopes
- `comanda-mutation-context.service.ts` — contexto transacional
...
- `operations-realtime.gateway.ts` — fachada do gateway e wiring do namespace
- `operations-realtime.socket-lifecycle.ts` — connect/disconnect, rooms e reconcile
- `operations-realtime.redis-transport.ts` — adapter Redis e fanout entre instancias
...
- o servidor suporta `websocket` e `polling`; o cliente web atual abre `websocket` com fallback `polling` e upgrade habilitado
