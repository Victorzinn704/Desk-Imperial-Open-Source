...
- imagens de produto passam por foto persistida, Open Food Facts e Pexels apenas para itens ilustrativos como combos/comida preparada
...
2. Open Food Facts via EAN/barcode
3. fallback ilustrativo apenas para combos/comida preparada
4. busca manual por Pexels quando o operador estiver cadastrando ou revisando item nao-embalado

Evidencia principal:

- resolucao de visual em `apps/web/lib/product-visuals.ts`
- catalogo nacional em `apps/web/lib/brazilian-packaged-beverage-catalog.ts` somente para identificar fonte/metadata, nao para gerar imagem fake
...
- bebidas embaladas sem foto real ficam como `sem foto`/iniciais; nao recebem packshot SVG nem foto generica de Pexels
- o site em producao e o banco sao a fonte canonica de imagem; fallback visual nao pode sobrescrever `Product.imageUrl`
