...
- responde webhook rapidamente e processa inbound via fila quando Redis esta disponivel
- responde com cards interativos via inline keyboard
- usa comandos e callbacks, nao apenas texto livre
- resolve texto livre conhecido por roteamento deterministico e pode usar Gemini como fallback opcional de intencao
...
### 8.1 Inbound Worker e Gemini Intent Route

O webhook do Telegram nao deve fazer trabalho pesado. O caminho atual e:

```text
Telegram -> Controller valida segredo -> Redis inbound queue -> TelegramInboundWorker -> TelegramBotService
```

Se Redis estiver indisponivel, existe fallback assincrono direto para nao perder mensagens, mas esse modo deve ser tratado como degradado.

Texto livre passa primeiro por regras deterministicas (`vendas`, `caixa`, `relatorio`, `equipe`, `alertas`, `portal`, `status`). O Gemini entra apenas como fallback opcional para mapear intencao para um comando suportado, sem conversar livremente com dados sensiveis.

Variaveis opcionais:

```env
TELEGRAM_GEMINI_INTENT_ENABLED=false
TELEGRAM_GEMINI_INTENT_MODEL=gemini-2.0-flash
TELEGRAM_GEMINI_INTENT_TIMEOUT_MS=1800
GEMINI_API_KEY=
```

Regras:

- Gemini nunca substitui validacao de permissao do bot;
- resposta do modelo precisa bater em comando permitido ou e descartada;
- timeout curto evita deixar o chat parecendo travado;
- cache Redis reduz custo e latencia para frases repetidas.
