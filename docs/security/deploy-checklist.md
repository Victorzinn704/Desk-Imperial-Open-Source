...
## 5. Publicacao open source

Publicar o codigo no GitHub publico e diferente de fazer deploy. Antes de sincronizar `origin/main` com `public/main`:

- seguir [open-source-sync-runbook.md](../operations/open-source-sync-runbook.md)
- criar branch `public-sync/*` baseada em `public/main`
- preservar backup de `public/main`
- revisar denylist operacional de Oracle, workstation, Telegram, Mercado Pago, QZ Tray e relatórios internos
- rodar `npm run repo:scan-public`
- rodar `npm run lint:secrets`
- rodar `npm run quality:contracts`
- abrir PR no repositório publico

Nunca publique direto em `public/main`.

---

## 6. Banco, migrations e BI
...
## 8. Realtime, mobile e notificacoes
