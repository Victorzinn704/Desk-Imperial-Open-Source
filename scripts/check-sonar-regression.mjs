...
function printUsage() {
  ...
  console.log("  node scripts/check-sonar-regression.mjs --current docs/release/sonarqube-local-strict-scan-2026-04-03.json");
  console.log("  node scripts/check-sonar-regression.mjs --previous docs/release/sonarqube-local-scan-2026-04-03.json --current docs/release/sonarqube-local-strict-scan-2026-04-03.json");
}
...
async function detectPreviousReport(currentPath) {
  ...
  const base = reports.find(
    (report) => BASE_REPORT_RE.test(report.name) && report.fullPath !== currentAbsolute,
  );
...
function getQualityGateFailures(report) {
  const conditions = Array.isArray(report?.qualityGate?.conditions)
    ? report.qualityGate.conditions
    : [];
